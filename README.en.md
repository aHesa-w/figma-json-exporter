...
2. **Document flow and semantic refactor**: the agent saves a revertible first version, reads `flow-plan.json` and `semantic-plan.json`, and refactors the real page from parent containers first, following the priority `block → inline/inline-block → flex → grid`. Ordinary vertical structure uses block flow and simple horizontal content uses inline/inline-block; Flex only for dynamic distribution, fill/stretch or wrapping; Grid only for two-dimensional alignment or an overlapping paint stack. Do not mechanically translate Auto Layout into Flex/Grid, and do not reproduce coordinates with per-child x/y margins. Code follows the safe `codeOrder` top-to-bottom then left-to-right; overlapping paint stacks keep design order. Framework targets turn repeated structures into data loops and plain HTML keeps expanded DOM with `d2c-repeat` comments; only `safe-local` interactions are implemented autonomously, `callback-only` must not invent routes, APIs or persistence. Keep layer IDs and the design hierarchy; unlabelled wrappers with a clear structural role may be added, but never pass validation by deleting layers or changing target coordinates.
