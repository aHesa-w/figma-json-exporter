...
export const GUIDANCE: GuidanceEntry[] = [
  // ── 流程 / 校验 ──────────────────────────────────────────────────────────
  {
    ...
    guidance: "Sequence: (1) figma_export and keep its compact summary, (2) open previewHtmlPath and read generationManifestPath first; inspect design.json, implementation.json and other plans only for targeted nodes/properties that the preview or validation says need work, (3) use the model-free preview as a starting artifact and implement every exported layer in document flow for the final version — with data-d2c-id and selection roots with data-d2c-root, (4) run the exported DOM collector and call figma_validate_layout phase=baseline until passed, (5) ensure the final version satisfies flow-plan, (6) remeasure at the same viewport and call phase=flow with the successful baselineReportPath until workflowComplete=true, (7) review visual/interaction fidelity separately. The starter preview itself is not required to pass flow. Load tag-specific standards with figma_guidance rather than assuming unread rules.",
  },
  {
    tag: "baseline",
