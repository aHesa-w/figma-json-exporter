...
export function renderingRequirements(node: Layer) {
  ...
  if (node.renderAs === "image") {
    ...
    return { checks, review, rules: ["Keep data-d2c-id on an imageBounds-sized wrapper positioned at the visual canvas origin with overflow:visible. Place exactly one IMG marked data-d2c-asset=assetId at 0/0 with the same width/height; do not fit the enlarged canvas into layout bounds. Do not give the asset IMG another layer ID. Local paint, opacity, radius, clipping and effects are baked; preserve ancestor clipping/opacity and mask relationships."] };
  }
...
export function validateRendering(node: Layer, found: ActualLayer, design: Design, tolerance: number): PropertyMismatch[] {
  ...
  if (node.renderAs === "image") {
    if (node.imageBounds && node.relativeImageBounds) {
      ...
      if (found.tagName !== "IMG" && (style?.overflowX !== "visible" || style?.overflowY !== "visible" || style?.clipPath !== "none" || style?.maskImage !== "none" || /\b(paint|strict|content)\b/.test(style?.contain ?? ""))) fail("raster-wrapper-clipping", "Do not clip the exported visual canvas on its image-sized wrapper", style);
    }
    return mismatches;
  }
